...
if (isset($_POST['add_room'])) {
    ...
    $values = [
        $frm_data['name'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'],
...
if (isset($_POST['get_all_rooms'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn  btn-outline-info  btn-sm shadow-none' data-bs-toggle='modal' data-bs-target='#room-images'>
...
if (isset($_POST['get_room'])) {
    ...
    $res3 = select("SELECT * FROM `room_features` WHERE `room_id`=?", [$frm_data['get_room']], 'i');
    ...
    if (mysqli_num_rows($res3) > 0) { // Use $res3 to fetch features data
        while ($row = mysqli_fetch_assoc($res3)) { // Use $res3 here
...
if (isset($_POST['get_room_images'])) {
    ...
    $frm_data = filteration($_POST);
        $res4 = select("SELECT * FROM `room_images` WHERE `room_id`=?", [$frm_data['get_room_images']], 'i');
    ...
    while ($row = mysqli_fetch_assoc($res4)) 
    {
        ...
        if($row['thumb'] == 1){
            $thumb_btn = "<i class='bi bi-check text-light bg-success px-2 py-1 rounded fs-5'></i>";
        ...
        echo <<<data
                ...
                <td><img src='$path$row[image]' class=' img-fluid ' width = "350px"></td>
                <td>$thumb_btn</td>
                <td>
...
if (isset($_POST['remove_room'])) {
   ...
   $res2 = delete("DELETE FROM `room_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
   $res2 = delete("DELETE FROM `room_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
   $res2 = delete("UPDATE `rooms` SET `removed`=? WHERE `id`=?",[1,$frm_data['room_id']],'ii');
