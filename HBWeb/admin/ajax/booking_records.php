...
//     $res = select($query, [$searchParam, $searchParam, $searchParam], 'sss');

//     $limit_query = $query . " LIMIT $start,$limit";
//     $limit_res = select($limit_query, [$searchParam, $searchParam, $searchParam], 'sss');
...
//                 <td>
//                 <button type='button' onclick='download($data[booking_id])' class='btn btn-sm fw-bold btn-info mt-2'>
...
//                         <button onclick='change_page($i)' class='page-link shadow-none'>$i</button></li>";
//         }
...
if(isset($_POST['get_bookings']))
  {
    ...
    OR (bo.booking_status='cancelled' AND bo.refund=1)
    OR (bo.booking_status='payment failed')) 
    AND (bo.order_id LIKE ? OR bd.phonenum LIKE ? OR bd.user_name LIKE ? OR bd.room_name LIKE ?) 
    ORDER BY bo.booking_id DESC";
