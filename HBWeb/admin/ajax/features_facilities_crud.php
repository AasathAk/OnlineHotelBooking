    ...
    if ($res) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            echo <<<data
                  ...
                  <td>$i</td>
                  <td>$row[name]</td>
                  <td><button type="button" onclick="rem_features($row[id])" class="btn btn-danger btn-sm shadow-none">
        ...
        echo 'db_error'; // Database query failed
    }
}
    ...
    if ($res) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            echo "<tr class='align-middle'>
                    <td>$i</td>
                    <td><img src='$path{$row['icon']}' width='90px' alt='Facility Icon'></td>
                    <td>{$row['name']}</td>
